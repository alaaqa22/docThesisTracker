...
public interface UserDAO {
	 ...
	 * present in the database already). All entries in {@code user.userState} will
	 * be written into the Database.
	 * 
	 * If the user cannot be added to the database (due to invalid input or other
	 * error), an exception is thrown.
	 * 
	 * @param user        The user DTO to be added to the Database
	 * @param transaction The transaction associated with this operation.
	 * @throws DataNotCompleteException if necessary values of the {@code user} are
	 *                                  not set
	 ...
	 * 
	 * <p>
	 * Only the {@code user.id} property of {@code user} needs to be set, all other
	 * values will be ignored. If the unique Identifier was not found in the
	 * Database, an exception is thrown.
	 * 
	 ...
	 * 
	 * <p>
	 * The {@code user.id} needs to be set correctly. All other User Properties will
	 * be overwritten unless set to {@code null}. The email address must be unique
	 * (i.e., not present in the database already). TODO All entries in
	 * {@code user.userState} will update or create the table entries with the key
	 * specified by the faculty field of the userState map and the ID of the user.
	 * 
	 * 
	 ...
	 * <p>
	 * Only the {@code user.id} property of {@code user} needs to be set, all other
	 * values will be overwritten when retrieving the User from the Database. If the
	 * unique Identifier was not found in the Database, an exception is thrown.
	 * 
	 ...
	 * other values will be overwritten when retrieving the User from the Database.
	 * If the unique Email was not found in the Database, {@code false} is returned.
	 * 
	 * @param user        The user DTO with set email value to be retrieved from the
	 *                    Database
	 * @param transaction The transaction associated with this operation.
	 * @return {@code true} if user with that same email was found in the Database;
	 *         {@code false} if no user with that email was found
	 */
	 ...
	 * used to filter results if set. {@code user.id} and {@code user.userState} are
	 * ignored in the filter object. To filter for a specified faculty,
	 * {@code faculty.id} needs to be set in the faculty DTO.
	 * 
	 * 
	 ...
	 *                    {@code null}, no filter.
	 * @param auth        can be {@code null}. Filter used for authorization level.
	 *                    If {@code null}, not filtered.
	 * @param transaction The transaction associated with this operation.
	 ...
	 * {@code user.userState} along with the {@code user.id} will be added to the
	 * database, or updated if existing.
	 * 
	 * @param User        The user DTO containing the user ID and map of faculties
	 *                    with authorization levels.
	 * @param transaction The transaction associated with this operation.
	 * @throws DataNotCompleteException if necessary values of the {@code user} are
	 *                                  not set
	 * @throws InvalidInputException    if input data is faulty
	 ...
	 * <p>
	 * The {@code user.id} needs to exist and be set correctly, as well as the
	 * faculty IDs in {@code user.userState}. All user ID and faculty ID pairs need
	 * to exist in the authorization database. All entries from the
	 * {@code user.userState} along with the {@code user.id} will be removed from
	 * the database. TODO rmAuth consider changing input parameters to more usable
	 * ones.
	 * 
	 * @param User        The user DTO containing the user ID and map of faculties
	 *                    with authorization levels.
	 ...
	 */
	public void removeAuth(User user, Transaction transaction)
			throws DataNotFoundException, InvalidInputException, DataNotCompleteException;

	/**
	 * Add user to admin table.
	 * 
	 * <p>
	 * The {@code user.id} needs to exist in the user table and be set correctly and
	 * not yet in the admin database, all other fields will be ignored.
	 * 
	 * @param User        The user DTO containing the ID to be added to the admin
	 *                    table
	 * @param transaction The transaction associated with this operation.
